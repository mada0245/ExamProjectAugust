<script>
    ...
    function setTimer(){
        ...
        for (let i = 0; i < todaysRecepies.length; i++) {
            const foundRecepie = recepies.find((recepies) => recepies.name === todaysRecepies[i]);
            todaysRecepiesPreparationTime = todaysRecepiesPreparationTime + foundRecepie.preparationTime;
        } 
        ...
        function countdown() {
            if (timer >= 0) {
                setTimeout(() => {
                    toastr.warning(`The order number ${newOrderNumber} will take ${timer} seconds to be prepared`, "⌚️⌚️⌚️⌚️⌚️");
                    toastr.clear();
                    timer--;
                    countdown();
                }, 1000);
            }else{
                toastr.success(`The order number ${newOrderNumber} is ready`, "😊😊😊😊😊");
                toastr.clear();
            }
        }
